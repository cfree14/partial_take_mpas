...
data2_formatted <- data2 %>%
  ...
  # Order variables
  mutate(variable=as.character(variable),
         variable=factor(variable, levels=var_order$variable))
...
# Plot regression
g1 <- ggplot(data1_orig, aes(x=mgt.sum, y=effect)) +
  # Reference line
...
# Plot effect sizes
g2 <- ggplot(data2_formatted, aes(y=variable, x=effect, color=level)) +
  # Reference line
