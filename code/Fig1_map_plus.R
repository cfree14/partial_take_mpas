...
# Order
type_order <- c("No-take (NT)", "Multi-use (MU)", "Non-MPA")

# Reefs
nreefs  <- 100
reefs <- tibble(lat_dd=runif(nmpas, min=-65, max=80),
                long_dd=runif(nmpas, min=-180, max=180),
                type=sample(x=c("No-take (NT)", "Multi-use (MU)", "Non-MPA"), size=nreefs, replace=T) %>%
                  as.factor(., levels=type_order))

# Effects
effects <- tibble(type=c('No-take', "Multi-use"),
                  mean=c(0.45, 0.2),
                  sd1=0.2,
                  sd2=0.3) %>%
...
g1 <- ggplot() +
  ...
  # Plot points
  geom_point(data=reefs, mapping=aes(x=long_dd, y=lat_dd, fill=type), pch=21) +
  # Labels
  labs(x=" ", y="", tag="A",) +
  # Crop
  coord_sf(y=c(-65,85)) +
  # Legend
  scale_fill_manual(name="Reef type", values=c("red", "blue", "grey60")) +
  # Theme
  theme_bw() + base_theme +
...
g2 <- ggplot(effects, mapping=aes(x=type, y=mean, color=type)) +
  ...
  # Legend
  scale_color_manual(name="Reef type", values=c("red", "blue")) +
  # Theme
  theme_bw() + base_theme +
